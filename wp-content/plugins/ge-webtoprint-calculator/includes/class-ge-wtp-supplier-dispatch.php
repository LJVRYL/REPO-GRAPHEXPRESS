...
final class GE_WTP_Supplier_Dispatch {
    ...
    public static function render_order( $order ) {
        ...
        $artwork_ready = ! class_exists( 'GE_WTP_Artwork_Library' ) || GE_WTP_Artwork_Library::order_ready_for_dispatch( $order );
        ?>
    ...
    private static function send_email( $order, $profile ) {
        ...
        $production_items = class_exists( 'GE_WTP_Production' ) ? GE_WTP_Production::actionable_items( $order, false ) : $order->get_items( 'line_item' );
        if ( ! $production_items ) { return false; }
        foreach ( $production_items as $item ) { $rows .= '<tr><td style="padding:10px;border-bottom:1px solid #ddd"><strong>' . esc_html( $item->get_name() ) . '</strong><br>' . wp_kses_post( wc_display_item_meta( $item, array( 'echo' => false, 'separator' => ' · ' ) ) ) . '</td><td style="padding:10px;border-bottom:1px solid #ddd;text-align:right">' . esc_html( $item->get_quantity() ) . '</td></tr>'; }
