...
final class GE_WTP_Customers {
    ...
    public static function render_staff() {
        ...
        ?><div class="ge-staff-heading"><div><span>Clientes</span><h1>Perfiles de clientes</h1><p>Contactos, destinos y contexto comercial en un solo lugar.</p></div></div><section class="ge-admin-panel"><div class="ge-admin-panel-head"><div><span><?php echo esc_html( count( $users ) ); ?> perfiles</span><h2>Clientes registrados</h2></div></div><?php if ( ! $users ) : ?><div class="ge-admin-empty">Todavía no hay clientes registrados.</div><?php else : ?><div class="ge-admin-table-scroll"><table class="ge-admin-table"><thead><tr><th>Cliente</th><th>Email</th><th>WhatsApp</th><th>Empresa</th><th>Direcciones</th><th></th></tr></thead><tbody><?php foreach ( $users as $user ) : ?><tr><td><div class="ge-customer-cell"><span class="ge-customer-thumb"><?php echo self::avatar_markup( $user->ID, 38 ); ?></span><span><strong><?php echo esc_html( trim( $user->first_name . ' ' . $user->last_name ) ?: $user->display_name ); ?></strong><small><?php echo esc_html( implode( ', ', $user->roles ) ); ?></small></span></div></td><td><?php echo esc_html( $user->user_email ); ?></td><td><?php echo esc_html( get_user_meta( $user->ID, '_ge_whatsapp', true ) ?: '—' ); ?></td><td><?php echo esc_html( get_user_meta( $user->ID, 'billing_company', true ) ?: '—' ); ?></td><td><?php echo esc_html( count( self::addresses( $user->ID ) ) ); ?></td><td><a href="<?php echo esc_url( GE_WTP_Staff_Portal::portal_url( 'customers', array( 'customer_id' => $user->ID ) ) ); ?>">Ver ficha →</a></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?></section><?php
    }
}
