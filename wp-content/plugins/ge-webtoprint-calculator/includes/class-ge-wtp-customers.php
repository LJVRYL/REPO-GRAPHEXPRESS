...
final class GE_WTP_Customers {
    ...
    public static function render_for_portal( $user_id = 0, $read_only = false ) {
        echo self::profile_form( $user_id ? absint( $user_id ) : get_current_user_id(), 'markcom', false, $read_only );
    }

    public static function profile_form( $user_id, $context = 'account', $staff = false, $read_only = false ) {
        ...
        <section class="ge-customer-profile">
            ...
            <form method="post" enctype="multipart/form-data" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
                ...
                <?php if ( $read_only ) : ?><fieldset disabled class="ge-profile-preview-fields"><?php endif; ?>
                ...
                <?php if ( $read_only ) : ?></fieldset><div class="ge-profile-actions"><span>Vista previa protegida: los datos no se pueden modificar desde acá.</span></div><?php else : ?><div class="ge-profile-actions"><button type="submit"><?php echo $staff ? 'Guardar ficha del cliente' : 'Guardar mis datos'; ?></button><span>Los campos sin la indicación “obligatorio” son opcionales.</span></div><?php endif; ?>
            </form>
            <?php if ( ! $verified && ! $staff && ! $read_only ) : ?><form id="ge-resend-verification" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="ge_resend_customer_verification"><input type="hidden" name="profile_context" value="<?php echo esc_attr( $context ); ?>"><?php wp_nonce_field( 'ge_resend_customer_verification' ); ?></form><?php endif; ?>
        </section>
        <?php
        return ob_get_clean();
    }
